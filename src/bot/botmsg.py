...
class BotMsg(object):
    ...
    def packstat(self, _lev, _rec):
        ...
        try:
            ...
            if self.cfg.tracking:
                self.log.track(_lev+14, "utcnow:          " +
                               str(utcnow), True)
                self.log.track(_lev+14, "datnow:          " +
                               str(datnow), True)
                self.log.track(_lev+14, "datnow_yr:       " +
                               str(datnow_yr), True)
                self.log.track(_lev+14, "startdate:       " +
                               str(startdate), True)
                self.log.track(_lev+14, "utcstart:        " +
                               str(utcstart), True)
            ...
            heading_deg = float(heading_raw * 100.0)
            heading_adj = int(math.floor(heading_deg))      # s/b <= 16 bits

            pack1b32 = (int(segid) << 29) + \
                (int(statid_adj) << 16) + int(heading_adj)

            if self.cfg.tracking:
                self.log.track(_lev+14, "segid:          " + str(segid), True)
                self.log.track(_lev+14, "statid_raw:     " +
                               str(statid_raw), True)
                self.log.track(_lev+14, "statid_adj:     " +
                               str(statid_adj), True)
                self.log.track(_lev+14, "heading_raw:    " +
                               str(heading_raw), True)
                self.log.track(_lev+14, "heading_deg:    " +
                               str(heading_deg), True)
                self.log.track(_lev+14, "heading_adj:    " +
                               str(heading_adj), True)
                self.log.track(_lev+2, "pack1b32(bin):  " +
                               str(bin(pack1b32)[2:].zfill(32)), True)
                self.log.track(_lev+2, "pack1b32(int):  " +
                               str(int(pack1b32)), True)
                self.log.track(_lev+2, "pack1b32(hex):  " +
                               str(hex(int(pack1b32))[2:].zfill(8)), True)
            ...
            if longi_raw < 0.0:
                ...
                longi_mic = float(longi_raw * -1000000.0)
            else:
            ...
            pack2b64 = (int(tstamp_adj) << 39) + (int(longi_sgn) <<
                                                  38) + (int(longi_adj) << 10) + (int(trig_wk_adj))

            if self.cfg.tracking:
                self.log.track(_lev+14, "tstamp_ms:      " +
                               str(tstamp_ms), True)
                self.log.track(_lev+14, "tstamp:         " + str(tstamp), True)
                self.log.track(_lev+14, "tstamp_adj:     " +
                               str(tstamp_adj), True)
                self.log.track(_lev+14, "longi_raw:      " +
                               str(longi_raw), True)
                self.log.track(_lev+14, "longi_mic:      " +
                               str(longi_mic), True)
                self.log.track(_lev+14, "longi_adj:      " +
                               str(longi_adj), True)
                self.log.track(_lev+14, "trig_wk_ct:     " +
                               str(trig_wk_ct), True)
                self.log.track(_lev+14, "trig_wk_adj:    " +
                               str(trig_wk_adj), True)

                self.log.track(_lev+2, "tstamp_adj(b):  " +
                               str(bin(tstamp_adj)), True)
                self.log.track(_lev+2, "longi_adj(b):   " +
                               str(bin(longi_adj)), True)
                self.log.track(_lev+2, "trig_adj(b):    " +
                               str(bin(trig_wk_adj)), True)

                self.log.track(_lev+2, "pack2b64(bin):  " +
                               str(bin(pack2b64)[2:].zfill(64)), True)
                self.log.track(_lev+2, "pack2b64(int):  " +
                               str(int(pack2b64)), True)
                self.log.track(_lev+2, "pack2b64(hex):  " +
                               str(hex(int(pack2b64))[2:].zfill(16)), True)
                self.log.track(
                    _lev+1, "Bitpack bits 64-127 (bytes 8-15).", True)
            ...
            pack3b32 = (int(batt_adj) << 25) + (int(bus_bit) << 20) + (int(bus_adj)
                                                                       << 19) + (int(temp_adj) << 12) + (int(node_adj) << 6) + (int(geo_adj))

            if self.cfg.tracking:
                self.log.track(_lev+14, "batt_adj(b):    " +
                               str(bin(batt_adj)), True)
                self.log.track(_lev+14, "bus_adj(b):     " +
                               str(bin(bus_adj)), True)
                self.log.track(_lev+14, "temp_adj(b):    " +
                               str(bin(temp_adj)), True)
                self.log.track(_lev+14, "node_adj(b):    " +
                               str(bin(node_adj)), True)
                self.log.track(_lev+14, "geo_adj(b):     " +
                               str(bin(geo_adj)), True)

                self.log.track(_lev+2, "pack3b32(bin):  " +
                               str(bin(pack3b32)[2:].zfill(32)), True)
                self.log.track(_lev+2, "pack2b32(int):  " +
                               str(int(pack3b32)), True)
                self.log.track(_lev+2, "pack3b32(hex):  " +
                               str(hex(int(pack3b32))[2:].zfill(8)), True)
                self.log.track(
                    _lev+1, "Bitpack bits 128-159 (bytes 16-19).", True)
            ...
            pack4b32 = (int(wake_adj) << 24) + (int(task_adj) << 18) + \
                (int(trig_adj) << 12) + (int(rule_adj) << 6) + int(sens_adj)

            if self.cfg.tracking:
                self.log.track(_lev+14, "wake_adj(b):    " +
                               str(bin(wake_adj)), True)
                self.log.track(_lev+14, "task_adj(b):    " +
                               str(bin(task_adj)), True)
                self.log.track(_lev+14, "trig_adj(b):    " +
                               str(bin(trig_adj)), True)
                self.log.track(_lev+14, "rule_adj(b):    " +
                               str(bin(rule_adj)), True)
                self.log.track(_lev+14, "sens_adj(b):    " +
                               str(bin(sens_adj)), True)

                self.log.track(_lev+2, "pack4b32(bin):  " +
                               str(bin(pack4b32)[2:].zfill(32)), True)
                self.log.track(_lev+2, "pack4b32(int):  " +
                               str(int(pack4b32)), True)
                self.log.track(_lev+2, "pack4b32(hex):  " +
                               str(hex(int(pack4b32))[2:].zfill(8)), True)
                self.log.track(
                    _lev+1, "Bitpack bits 160-191 (bytes 20-23).", True)
            ...
            if self.cfg.tracking:
                self.log.track(_lev+2, "pack5b32(bin):  " +
                               str(bin(pack5b32)[2:].zfill(32)), True)
                self.log.track(_lev+2, "pack5b32(int):  " +
                               str(int(pack5b32)), True)
                self.log.track(_lev+2, "pack5b32(hex):  " +
                               str(hex(int(pack5b32))[2:].zfill(8)), True)
                self.log.track(
                    _lev+1, "Bitpack bits 192-223 (bytes 24-27).", True)
            ...
            # latitide (-90.0-90.0)
            lati_raw = float(_rec[11])
            if lati_raw < 0.0:
                lati_sgn = 1                                 # sign bit is 1
                # latitude microdegrees
                lati_mic = float(lati_raw * -1000000.0)
            else:
            ...
            pack6b32 = (int(swinc_adj) << 28) + \
                (int(lati_sgn) << 27) + int(lati_adj)

            if self.cfg.tracking:
                self.log.track(_lev+14, "swinc_adj(b):   " +
                               str(bin(swinc_adj)), True)
                self.log.track(_lev+14, "lati_sgn(b):    " +
                               str(bin(lati_sgn)), True)
                self.log.track(_lev+14, "lati_adj(b):    " +
                               str(bin(lati_adj)), True)

                self.log.track(_lev+2, "pack6b32(bin):  " +
                               str(bin(pack6b32)[2:].zfill(32)), True)
                self.log.track(_lev+2, "pack6b32(int):  " +
                               str(int(pack6b32)), True)
                self.log.track(_lev+2, "pack6b32(hex):  " +
                               str(hex(int(pack6b32))[2:].zfill(8)), True)
                self.log.track(
                    _lev+1, "Bitpack bits 224-255 (bytes 28-31).", True)
            ...
            if self.cfg.tracking:
                self.log.track(_lev+14, "navsat_adj(b):  " +
                               str(bin(navsat_adj)), True)
                self.log.track(_lev+14, "evnt_adj(b):    " +
                               str(bin(evnt_adj)), True)

                self.log.track(_lev+2, "pack7b24(bin):  " +
                               str(bin(pack7b24)[2:].zfill(32)), True)
                self.log.track(_lev+2, "pack7b24(int):  " +
                               str(int(pack7b24)), True)
                self.log.track(_lev+2, "pack7b24(hex):  " +
                               str(hex(int(pack7b24))[2:].zfill(8)), True)
                self.log.track(_lev+1, "Combine Packing Increments.", True)

            buf1 = pack(self.fmtstat, pack1b32, pack2b64, pack3b32,
                        pack4b32, pack5b32, pack6b32, pack7b24)

            if self.cfg.tracking:
                self.log.track(_lev+2, "SR Pack Len: " +
                               str(len(str(buf1))), True)
                self.log.track(_lev+2, "SR Pack Hex: " +
                               str(buf1.encode("hex")), True)
        ...
        if self.cfg.tracking:
            ...
            self.log.track(_lev+2, "Now Msg Hex: " +
                           str(self.buf.encode("hex")), True)
    ...
    def packmeta(self, _lev, _rec, _tim, _idx):
        ...
        if self.cfg.tracking:
            ...
            self.log.track(_lev+1, "Cur Msg Hex: " +
                           str(self.buf.encode("hex")), True)
        ...
        if self.cfg.tracking:
            self.log.track(
                _lev+1, "Pack Common Header (bits 0-31; bytes 0-3).", True)
        ...
        if self.cfg.tracking:
            ...
            self.log.track(_lev+14, "dp_timestamp:  " +
                           str(dp_timestamp), True)
            self.log.track(_lev+14, "dp_node_type:  " +
                           str(dp_node_type), True)
            self.log.track(_lev+14, "dp_node_code:  " +
                           str(dp_node_code), True)
        ...
        try:
            pack1b32 = (int(segid) << 29) + (int(mtime_sgn) << 28) + (int(dp_timestamp) << 19) + (int(dp_node_code)
                                                                                                  << 13) + (int(dp_instance) << 9) + (int(dp_index) << 2) + (int(data_kind) << 1) + int(data_change)
        except Exception as e:
            enum = "MSG103"
        ...
        try:
            pack2b64 = 0
            pack2b64 = (int(statid_adj) << 52) + (int(heading_adj) << 36) + (int(pitch_sgn) << 35) + (int(pitch_adj) << 30) + (
                int(roll_sgn) << 29) + (int(roll_adj) << 24) + (int(channels_adj) << 20) + (int(dt_adj) << 16) + (int(size_adj) << 4)
        except Exception as e:
            enum = "MSG114"
        ...
        if not chg_eligible:
            if self.cfg.tracking:
                self.log.track(
                    _lev+1, "Bitpack 'std' bits 96-127 (bytes 12-15).", True)

            try:
                # Samples good as is
                samples_adj = int(_rec[27])
            ...
            if self.cfg.tracking:
                self.log.track(_lev+2,  "samples_adj: " +
                               str(samples_adj), True)
                self.log.track(_lev+2,  "rows_adj:    " + str(rows_adj), True)
                self.log.track(_lev+2,  "cols_adj:    " + str(cols_adj), True)

            try:
                pack3b32 = 0
                pack3b32 = (int(samples_adj) << 22) + \
                    (int(rows_adj) << 11) + int(cols_adj)
            except Exception as e:
                enum = "MSG114"
            ...
            if self.cfg.tracking:
                self.log.track(_lev+2, "pack3b32(bin):  " +
                               str(bin(pack3b32)[2:].zfill(32)), True)
                self.log.track(_lev+2, "pack3b32(int):  " +
                               str(int(pack3b32)), True)
                self.log.track(_lev+2, "pack3b32(hex):  " +
                               str(hex(int(pack3b32))[2:].zfill(8)), True)
            ...
            try:
                ...
                if self.cfg.tracking:
                    self.log.track(_lev+3, "std1(len):  " +
                                   str(len(str(std1))), True)
                    self.log.track(_lev+15, "std1(hex):  " +
                                   str(std1).encode("hex"), True)
                ...
                if self.cfg.tracking:
                    ...
                    self.log.track(_lev+15, "std2 Items:  " +
                                   str(int(len2)), True)
                    self.log.track(_lev+15, "std2 Type:   " +
                                   str(dt_fmt), True)
                    self.log.track(_lev+15, "std2 Format: " +
                                   str(dt_raw), True)
                    self.log.track(_lev+15, "std2 Bytes:  " +
                                   str(int(dt_byt)), True)

                std2 = ""
                for i in range(0, len2):
                    if self.cfg.tracking:
                        self.log.track(_lev+14, "Pack Item #" +
                                       str(i) + "  val: " + str(lst2[i]), True)
            ...
            if self.cfg.tracking:
                self.log.track(_lev+2, "DP Pack Len: " +
                               str(len(str(std3))), True)
                self.log.track(_lev+2, "DP Pack Hex: " +
                               str(std3).encode("hex"), True)
                self.log.track(
                    _lev+1, "Load Segment into the Uplink Message.", True)

        else:
            if self.cfg.tracking:
                self.log.track(
                    _lev+1, "Bitpack 'chg' DATA bits 96+ (bytes 12+).", True)
        ...
        if self.len + len(str(std3)) > self.cfg.max_msg_size:
            ...
            if self.cfg.tracking:
                self.log.track(_lev, str(enum) + ": " + str(emsg), True)
                self.log.track(_lev+1, "Max Msg Len: " +
                               str(self.cfg.max_msg_size), True)
                self.log.track(_lev+1, "Cur Msg Len: " + str(self.len), True)
                self.log.track(_lev+1, "DP Pack Len: " +
                               str(len(str(std3))), True)
