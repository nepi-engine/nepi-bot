    ...
    def initlog(self, _lev):
        ...
        if self.cfg.debugging > -1:
            ...
            if self.cfg.enum != None:
                ...
                self.track(0, "STARTING '" + str(self.name) +
                           "' DEBUGGING:", True)
        ...
        if self.cfg.logging > -1:
            ...
            if self.cfg.log_clear:
                ...
                if not os.path.exists(os.path.dirname(self.file)):
                    try:
                        ...
                        enum = "LOG002"
                        emsg = "initlog(): Can't Make Path: [" + \
                            str(e) + "]: Logging Turned OFF."
                        if self.cfg.tracking:
                            self.track(_lev+2, str(enum) +
                                       ": " + str(emsg), True)
            ...
            if self.cfg.logging > -1:
                self.track(0, "STARTING '" +
                           str(self.name) + "' LOGGING:", True)
                if self.name == "UNKNOWN":
                    enum = "LOG002"
                    emsg = "initlog(): Name should be either 'BOT-RECV' or 'BOT-SEND.'"
                    self.track(1, str(enum) + ": " + str(emsg), True)
        ...
        if self.cfg.tracking:
            ...
            if self.cfg.factory:
                ...
                self.track(_lev, "Using Bot Config File: " +
                           str(bot_cfg_file), True)
                self.track(_lev+13, "JSON Format: " +
                           str(self.cfg.bot_cfg_json), True)
            ...
            self.track(_lev+1, "python_compiler: " +
                       str(self.cfg.python_compiler), True)
            ...
            self.track(_lev+1, "lb_data_dir: " +
                       str(self.cfg.lb_data_dir), True)
            self.track(_lev+1, "lb_data_dir_path: " +
                       str(self.cfg.lb_data_dir_path), True)
            self.track(_lev+1, "data_zlib: " + str(self.cfg.data_zlib), True)
            self.track(_lev+1, "data_msgpack: " +
                       str(self.cfg.data_msgpack), True)
            ...
            self.track(_lev+1, "pipo_scor_wt: " +
                       str(self.cfg.pipo_trig_wt), True)
            self.track(_lev+1, "pipo_qual_wt: " +
                       str(self.cfg.pipo_qual_wt), True)
            self.track(_lev+1, "pipo_trig_wt: " +
                       str(self.cfg.pipo_trig_wt), True)
            self.track(_lev+1, "pipo_size_wt: " +
                       str(self.cfg.pipo_size_wt), True)
            self.track(_lev+1, "pipo_time_wt: " +
