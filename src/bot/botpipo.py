...
class BotPIPO(object):
    ...
    def __init__(self, _cfg, _log, _lev, _db):
        ...
        if self.cfg.tracking:
            ...
            self.log.track(_lev+1, "^scor_wt = " +
                           str(self.cfg.pipo_scor_wt), True)
            self.log.track(_lev+1, "^qual_wt = " +
                           str(self.cfg.pipo_qual_wt), True)
            self.log.track(_lev+1, "^size_wt = " +
                           str(self.cfg.pipo_size_wt), True)
            self.log.track(_lev+1, "^trig_wt = " +
                           str(self.cfg.pipo_trig_wt), True)
            self.log.track(_lev+1, "^time_wt = " +
    ...
    def initPIPO(self, _lev):
        ...
        try:
            ...
            if self.cfg.tracking:
                self.log.track(_lev+2, "ERROR: " + str(enum) +
                               ": " + str(emsg), True)
        ...
        if self.cfg.tracking:
            self.log.track(_lev+2, "^exec_age (min): " +
                           str(self.exec_age), True)
    ...
    def getPIPO(self, lev, metafile, metajson, trigger):
        ...
        sql = "SELECT * FROM node WHERE node_type = '" + \
            str(node_type) + "' AND node_instance = '" + \
            str(node_instance) + "' LIMIT 1"
        ...
        if not success[0] or not node_results:
            ...
            sql = "SELECT * FROM node WHERE node_type = '" + \
                str(node_type) + "' AND node_instance = '0' LIMIT 1"
            success, node_results = self.db.getResults(lev+2, sql, False)

            if not success[0] or not node_results:
                enum = "P122"
                emsg = "getPIPO(): [ Invalid/Inaccessible Node Type: " + \
                    str(node_type) + "]"
                if self.cfg.tracking:
                    self.log.errtrack(str(enum), str(emsg))
                return [False, str(enum), str(emsg)], None
            ...
            if self.cfg.tracking:
                ...
                self.log.track(
                    lev+1, "Create 'node' Table Entry for Instance.", True)
        ...
        if self.cfg.tracking:
            ...
            self.log.track(
                lev+1, "Get 'Standard' Name Keyword-Value Pair.", True)
        ...
        if std_data_type == "float64" or std_data_type == "int64":
            ...
            enum = "P017"
            emsg = "getPipo(): Unknown Data Type [" + \
                str(std_data_type) + "] encountered."
            if self.cfg.tracking:
                self.log.errtrack(str(enum), str(emsg))
            return [False, str(enum), str(emsg)], None
        ...
        if self.cfg.tracking:
            self.log.track(lev+1, "Std Head Bytes: " +
                           str(std_head_bytes), True)
            self.log.track(lev+1, "Std Data List:  " +
                           str(std_data_list), True)
            self.log.track(lev+1, "Std Data Items: " +
                           str(std_data_items), True)
            self.log.track(lev+1, "Std Data Type:  " +
                           str(std_data_type), True)
            self.log.track(lev+1, "Std Data Bytes: " +
                           str(std_data_bytes), True)
        ...
        if self.cfg.tracking:
            ...
            self.log.track(
                lev+1, "Get 'Change' Name Keyword-Value Pair.", True)
        ...
        try:
            ...
            if self.cfg.tracking:
                self.log.track(
                    lev+1, "No Change File Keyword Found; Continue.", True)
        ...
        if chg_file_name:
            ...
            if chg_file_name == "NC" or chg_file_name == "nc":
                if self.cfg.tracking:
                    self.log.track(
                        lev+1, "Keyword 'NC' Specified; Continue.", True)
            elif chg_file_name == "NULL" or chg_file_name == "Null" or chg_file_name == "null":
                if self.cfg.tracking:
                    self.log.track(
                        lev+1, "Keyword 'NULL' Specified; Continue.", True)
            else:
                ...
                if not success[0]:
                    ...
                    try:
                        ...
                        if chg_data_type == "float64" or chg_data_type == "int64":
                            ...
                            if self.cfg.tracking:
                                self.log.track(
                                    lev+1, "Unknown Data Type [" + str(chg_data_type) + "] encountered.", True)
                                self.log.track(
                                    lev+1, "Ignore; Continue.", True)
                    except Exception as e:
                        # One last switch back on final error.
                        chg_eligible = False
                        if self.cfg.tracking:
            ...
            if self.cfg.tracking:
                self.log.track(lev+1, "Chg ELIGIBLE:   " +
                               str(chg_eligible), True)
                self.log.track(lev+1, "Chg Head Bytes: " +
                               str(chg_head_bytes), True)
                self.log.track(lev+1, "Chg Data List:  " +
                               str(chg_data_list), True)
                self.log.track(lev+1, "Chg Data Items: " +
                               str(chg_data_items), True)
                self.log.track(lev+1, "Chg Data Type:  " +
                               str(chg_data_type), True)
                self.log.track(lev+1, "Chg Data Bytes: " +
                               str(chg_data_bytes), True)
                self.log.track(lev+1, "Chg MSG Size:   " +
                               str(chg_msg_size), True)
        ...
        if self.cfg.tracking:
            self.log.track(
                lev, "Compute 'Normalized' Data Product Size Factor.", True)
        ...
        if not success[0]:
            ...
            if self.cfg.tracking:
                self.log.track(
                    lev, "Data Product NOT ELIGIBLE for Uplink.", True)
    ...
    def computeNormalized(self, _lev, _std, _chg, _eli):
        ...
        f_norm = 1.0 - ((seg_size * 1.0) /
                        ((self.cfg.max_msg_size - 32) * 1.0))
    ...
    def computeDenominator(self, _lev, _ts):
        if self.cfg.tracking:
            self.log.track(
                _lev, "Entering 'computeDenominator() Module.", True)
            self.log.track(_lev+1, "_lev: " + str(_lev), True)
        ...
        try:
            ...
            emsg = "computeDenominator(): [" + str(e) + "]"
            if self.cfg.tracking:
                self.log.track(_lev+1, "ERROR: " + str(enum) +
                               ": " + str(emsg), True)

            return [False, str(enum), str(emsg)], 0.5

        if self.cfg.tracking:
            self.log.track(_lev+1, "System Time:      " +
                           str(float(_ts)), True)
            self.log.track(_lev+1, "Exec Age (min):   " +
                           str(self.exec_age), True)
            self.log.track(_lev+1, "Meta Age (min):   " + str(meta_age), True)
            self.log.track(_lev+1, "PIPO Age (min):   " + str(pipo_age), True)
            self.log.track(_lev+1, "PIPO Denominator: " +
                           str(denominator), True)
