...
class BotComm(object):
    ...
    #-------------------------------------------------------------------
    def __init__(self, cfg, log, typ, lev):
        self.cfg = cfg
        self.log = log
        self.typ = typ
        if self.cfg.tracking:
            self.log.track(lev, "Created BotComm Class Object: ", True)
            self.log.track(lev+1, "log: " + str(self.log), True)
            self.log.track(lev+1, "cfg: " + str(self.cfg), True)
    ...
    def getconn(self, lev):
        ...
        if self.typ == 'Iridium':
            ...
            # Open the serial port to be open
            try:
                if not self.serialport.isOpen():
                    self.serialport.open()
                    self.log.track(lev, "Serial Port Connected", True)

                    command = [b'AT+CGMI', b'AT+CGMM', b'AT+CGMR', b'AT+CGSN']
                    
                    isu_name = self.acquire_response(command[0])
                    self.log.track(lev+1, "ISU Name: " + isu_name, True)
                    
                    isu_model_number = self.acquire_response(command[1])
                    self.log.track(lev+1, "ISU Model Number: " + isu_model_number, True)
                    
                    isu_version = self.acquire_response(command[2])
                    self.log.track(lev+1, "ISU Version: \n" + isu_version, True)
                    
                    isu_imei = self.acquire_response(command[3])
                    self.log.track(lev+1, "ISU IMEI: " + isu_imei, True)
    ...
    # Receive a Message.
    #-------------------------------------------------------------------
    def receive(self, lev):
        if self.cfg.tracking:
            self.log.track(lev, "Receive Message on Established Connection.", True)
            self.log.track(lev+1, "lev: " + str(lev), True)
    ...
    def close(self, lev):
        if self.cfg.tracking:
            self.log.track(lev, "Close the Communications Connection.", True)
            self.log.track(lev+1, "lev: " + str(lev), True)
